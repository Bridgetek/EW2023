...
def psi_kpa(psi = None, kpa= None):
    if psi != None: return 1.0*psi * 6.89475729
    if kpa != None: return 1.0*kpa / 6.89475729
    
def psi_bar(psi= None, bar= None):
    if psi != None: return 1.0*psi / 14.5037738
    if bar != None: return 1.0*bar * 14.5037738

def c_f(c= None, f= None):
    if c != None: return (1.0*c * 1.8) + 32
    if f != None: return 1.0*(f-32) / 1.8

def mm_inch(mm= None, inch= None):
    if mm   != None: return mm / 25.4
    if inch != None: return inch * 25.4
