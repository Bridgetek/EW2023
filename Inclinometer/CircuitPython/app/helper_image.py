...
from app.helper_widgets import *
...
class evehpi_pallete():
    def __init__(self, addr_lut, size_lut, addr_index, size_index):
...
class evehp_image():
    def __init__(self, offsetx, offsety, w, h, path, 
                 flash_addr=0, flash_size=0, layout=BrtEve.ASTC_4x4, 
                 tag=None, paletted:evehpi_pallete=None) -> None:
        self.w = w
        self.h = h
        ...
        self.filepath = path
        self.addr_ramg = 0
        self.addr_flash = evehp_math_flash_address_convert(flash_addr)
...
def evehp_image_draw(eve: BrtEve, img: evehp_image, x=0, y=0, tag=None, scale_ratio=1, default_ramg=True, msg_on=False, addr=None, rotate_angle=0):
    # print("name", img.filepath, "where", "flash" if img.exist_on_flash else "ramg")

    if img.exist_on_flash:
        return _asset_draw_flash(eve, img, x, y, tag, scale_ratio)
    elif img.exist_on_ramg:
...
def evehp_image_copy_to_ramg(eve: BrtEve, img: evehp_image, msg_on=True, addr=None):
    ...
    if address > eve.RAM_G_SIZE:
        evehp_widget_message(eve, "Error: image exceeded RAM_G size")
        return False
...
def _asset_draw_flash(eve: BrtEve, img: evehp_image, x=0, y=0, tag=None, scale_ratio=1):
    if img.paletted:
        ...
        eve.cmd_setbitmap(img.addr_flash, img.bitmaplayout, img.w, img.h)
...
def _asset_draw_ramg(eve: BrtEve, img: evehp_image, x=0, y=0, tag=None, scale_ratio=1, rotate_angle=0):
    _MAX_ANGLE       =  360
    _CIRCLE_MAX      =  65536
    mrootx = 0
    mrooty = 0
