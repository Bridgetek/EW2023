...
class BrtEveRP2040():
    ...
    """
    def eve_power_cycle(self, is_up):
        if is_up:
            self.pin_eve_pdn = 0
            time.sleep(.02)
            self.pin_eve_pdn = 1
    ...
    def __init__(self, 
        ...
        match = os.uname().machine # pylint: disable=no-member
        if match != 'Raspberry Pi Pico with rp2040':
            print("Error: Noe detected an RP2040, os name = ", match)
            return
    ...
    def configure_spi(self):
        """ Setup SPI interface"""
        self.spi_eve.configure(baudrate=60000000, phase=0, polarity=0)
